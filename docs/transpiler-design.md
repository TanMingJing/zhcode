...
### 设计决策：**支持英文标点符号，不支持中文标点**

**原因：**
1. **编程语言通用惯例** - 所有主流编程语言都使用英文标点
2. **键盘输入友好** - 用户无需频繁切换输入法
3. **兼容性** - 避免编码问题（UTF-8 vs ASCII）
4. **工具链支持** - 编辑器、IDE、Git等都针对英文标点优化
...
令 x = 10；           // 中文分号
函数 add（a，b）{ }   // 中文括号和逗号
const obj = { 名字： "张三" };  // 中文冒号

// ✅ 必须使用
令 x = 10;           // 英文分号
函数 add(a, b) { }   // 英文括号和逗号
const obj = { 名字: "张三" };  // 英文冒号
```
